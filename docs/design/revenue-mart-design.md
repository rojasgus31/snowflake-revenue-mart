...
| D6 | Non-positive quantity | 2 rows: `ORD90103` quantity -15, `ORD90104` quantity 0 | **Reject both.** `INVALID_QUANTITY` | The source has no return or credit-memo type, so a negative quantity is unexplained rather than meaningful, and a zero-quantity order carries no revenue to recognise. Both are retained under one reason; the quantity column distinguishes them. |
